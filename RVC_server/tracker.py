...
#     if np.all(heights >= 40) and width > 20:
#         return True, width
#     return False, 0
...
#         if current_time - self.last_greeting_time > 3:  # 마지막 인사 이후 30초 이상 지났는지 확인
#             if self.websocket:
...
class Tracker:
    def __init__(self, loop):
        ...
        self.last_human_detection_change_time = 0
        self.human_detection_change_count = 0
    ...
    async def send_greeting_message(self):
        current_time = time.time()
        if current_time - self.last_greeting_time > 3:  # 마지막 인사 이후 3초 이상 지났는지 확인
            if self.websocket:
                await self.websocket.send("Greeting Detected")
                self.last_greeting_time = current_time  # 현재 시간을 마지막 인사 감지 시간으로 업데이트
            # print("Greeting Detected")
    ...
    async def send_human_detection(self, detected):
        current_time = time.time()
        # 사람 감지 상태가 변경되었는지 확인
        if detected != self.human_detected:
            # 상태 변경 후 일정 시간(예: 2초)이 지났는지 확인
            if current_time - self.last_human_detection_change_time > 3:
                self.human_detected = detected
                message = "Human detected" if detected else "Human undetected"
                if self.websocket:
                    await self.websocket.send(message)
                self.last_human_detection_change_time = current_time
                self.human_detection_change_count = 0
            else:
                # 너무 자주 상태 변경이 발생하면 카운트 증가
                self.human_detection_change_count += 1
                if self.human_detection_change_count > 1:  # 너무 자주 변경되는 것을 감지
                    # 너무 자주 변경되는 경우 처리
                    pass
    ...
    def start(self):
        while self.cap.isOpened():
            ...
            human_detected = results_pose.pose_landmarks is not None

            asyncio.run_coroutine_threadsafe(
                self.send_human_detection(human_detected), self.loop
            )
