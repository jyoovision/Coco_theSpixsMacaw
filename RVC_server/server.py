...
async def echo(
    ...
    try:
        async for message in websocket:
            ...
            #  Human Detected 메시지가 오면 언리얼로 메시지 전송
            elif message == "Human detected":
                isHuman = True
                if "unreal" in connected_clients:
                    send_detected_task = asyncio.create_task(
                        connected_clients["unreal"].send("Human Detected")
                    )
                    send_random_response_of_task = asyncio.create_task(
                        send_random_response_of(
                            connected_clients["unreal"],
...
async def main():
    ...
    asyncio.get_event_loop().run_in_executor(None, tracker.start)
